...
def app():
    ...
    #        result_jd = Job_recomm(CL)
        ...
        def get_location():
            f1=open('./india_districts.txt','r', errors = 'ignore')
            text1=f1.read()
...
#             recommendation = pd.DataFrame(columns = ['JobID',  'title', 'career level', 'company', 'industry', 'salary', 'location', 'webpage','score'])
            recommendation = pd.DataFrame(columns = ['JobID',  'job description','title','company overview', 'location','industry', 'salary','career level','score'])
            count = 0
            for i in top:
        #         recommendation.at[count, 'ApplicantID'] = u
                ...
                recommendation.at[count, 'company overview'] = df['company overview'][i]
...
#         merge1 = knn[['JobID','title','career level','company','location','industry','salary','webpage','score']].merge(TF[['JobID','score']], on= "JobID")
        merge1 = knn[['JobID',  'job description','title','company overview', 'location','industry', 'salary','career level','score']].merge(TF[['JobID','score']], on= "JobID")
        ...
        def Job_recomm(x):
            final_ = final[['JobID',  'job description','title','company overview', 'location','industry', 'salary','career level']]
    ...
    #        result_jd = Job_recomm(CL)
            ...
            final_df=final_jobrecomm[['JobID',  'job description','title','company overview', 'location','industry', 'salary','career level']]
